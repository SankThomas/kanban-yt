...
export default function EditTaskModal({ task, onClose, theme }) {
  const [title, setTitle] = useState(task.title || "");
  const [description, setDescription] = useState(task.description || "");
  const [priority, setPriority] = useState(task.priority || "medium");
  const [subtasks, steSubtasks] = useState(task.subtasks || []);
  ...
  const handleAddSubtask = () => {
    steSubtasks([...subtasks, { text: "", completed: false }]);
  };
  ...
  const handleSubtaskChange = (index, value) => {
    const updated = subtasks.map((subtask, i) =>
      i === index ? { ...subtask, text: value } : subtask
    );
    steSubtasks(updated);
  };
